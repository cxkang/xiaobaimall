    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/loT/ttopic/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.dataUuid = data.tTopic.dataUuid
                this.dataForm.name = data.tTopic.name
                this.dataForm.remark = data.tTopic.remark
                this.dataForm.createTime = data.tTopic.createTime
                this.dataForm.updateTime = data.tTopic.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/loT/ttopic/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
